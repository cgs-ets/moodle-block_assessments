...
/**
 * Provides the block_assessments/control module
 *
...
/**
 * @module block_assessments/control
 */
define(['jquery', 'core/log', 'core/pubsub', 'core/ajax', 'core/str'], function($, Log, PubSub, Ajax, Str) {
    ...
    function init(instanceid, date, displaypreference, userid, title) {
        Log.debug('block_assessments/control: initializing controls of the assessments block instance ' + instanceid);

        var region = $('[data-region="block_assessments-instance-' + instanceid + '"]').first();

        if (!region.length) {
            Log.debug('block_assessments/control: wrapping region not found!');
            return;
        }
    ...
    /**
     * Controls a single assessments block instance contents.
     *
     * @constructor
     * @param {jQuery} region
     */
    ...
    DailyTimetableControl.prototype.setupEvents = function () {
        ...
        $('#inst' + self.instanceid + '.block_assessments').unbind();
        
        // Set up navigation events.
        $('#inst' + self.instanceid + '.block_assessments').on('click', '.timetable-prev', function () {
            self.navigate(0);
        });
        $('#inst' + self.instanceid + '.block_assessments').on('click', '.timetable-next', function () {
            self.navigate(1);
        });

        // Hide timetable
        $('#inst' + self.instanceid + '.block_assessments').on('click', '.timetable-invisible', function () {
        ...
        // Show timetable
        $('#inst' + self.instanceid + '.block_assessments').on('click', '.timetable-visible', function () {
    ...
    DailyTimetableControl.prototype.navigate = function (direction) {
        ...
        Log.debug('block_assessments/content: Navigate timetable: ' + JSON.stringify(args));
        Ajax.call([{
            methodname: 'block_assessments_get_timetable_html_for_date',
            args: args,
           ...
           fail: function(reason) {
                Log.error('block_assessments. NavigateNextDay: Unable to get timetable.');
                Log.debug(reason);
    ...
    DailyTimetableControl.prototype.savePreferences = function(value)    {
        ...
        var preferences = [{
            'name': 'block_assessments_collapsed',
            'value': value,
            'userid': self.userid,
        }];
    ...
    DailyTimetableControl.prototype.refreshLayout = function(htmlResult){
        ...
        self.region.fadeOut(300, function() {
            ...
            self.region.show();
            self.region = $('[data-region="block_assessments-instance-' + self.instanceid +'"]');
            self.setupEvents();
    ...
    DailyTimetableControl.prototype.timetableUnavailable = function() {
        ...
        var unavailablestr = Str.get_string('timetableunavailable', 'block_assessments');
